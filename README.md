...
\## Implementation: 94-Page PDF Processing System
...
│       ├── knowledge\_graph.pkl    # 490-node knowledge graph

│       ├── communities.json       # 73 communities

│       └── entity\_extractions.json

├── src/pipeline/                   # Core pipeline files

│   ├── ambedkargpt.py            # Original pipeline

│   ├── ambedkargpt\_enhanced.py   # Enhanced version (latest)

│   ├── test\_interactive.py       # Interactive tester

│   ├── test\_simple.py           # Simple tester

│   └── test\_multiple\_questions.py
...
python -m venv semrag\_env

semrag\_env\\Scripts\\activate  # Windows
...
&nbsp;  Score\_local = 0.6 × ChunkSimilarity + 0.4 × EntitySimilarity
...
&nbsp;  Score\_global = 0.5 × CommunitySimilarity + 0.5 × ChunkSimilarity
...
from ambedkargpt\_enhanced import AmbedkarGPT
...
results = ambedkargpt.combined\_search(question)

answer = ambedkargpt.generate\_answer(question, results\['combined\_results'])
...
python test\_multiple\_questions.py
